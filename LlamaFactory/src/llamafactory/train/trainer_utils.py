...
def patch_deepspeed_count_used_parameters() -> None:
    r"""Patch DeepSpeed's backward-graph counting to tolerate tensors without grad accumulators."""
    try:
    ...
    if getattr(ds_utils, "_llamafactory_count_used_parameters_patched", False):
        return

    try:
        import deepspeed.runtime.zero.stage_1_and_2 as ds_stage12
    except ImportError:
        ds_stage12 = None

    def safe_count_used_parameters_in_backward(parameters) -> int:
        assert ds_utils.check_internal_apis_for_count_used_parameters(), (
        ...
        for param in parameters:
            ...
            try:
                ...
                # Some ZeRO-partitioned parameters can behave like inference tensors here.
                skipped_params += 1
                continue
        ...
        if skipped_params:
            logger.warning_rank0_once(
                "Patched DeepSpeed parameter counting skipped tensors without grad accumulators."
            )
    ...
    ds_utils._llamafactory_count_used_parameters_patched = True
    ds_stage3.count_used_parameters_in_backward = safe_count_used_parameters_in_backward
    if ds_stage12 is not None:
        ds_stage12.count_used_parameters_in_backward = safe_count_used_parameters_in_backward

    logger.info_rank0("Patched DeepSpeed count_used_parameters_in_backward for ZeRO compatibility.")
