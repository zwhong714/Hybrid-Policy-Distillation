...
class FinetuningArguments(
    ...
    use_hpd_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the HPD loss."},
    )
    hpd_sample_in_fp32: bool = field(
        default=False,
        metadata={"help": "Whether to upcast student logits to fp32 before HPD sampling."},
    )
    use_akl_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the AKL loss."},
    )
    akl_mu: float = field(
        default=0.5,
        metadata={"help": "The cumulative teacher probability threshold used by AKL to split head and tail tokens."},
    )
    akl_ce_weight: float = field(
        default=1.0,
        metadata={"help": "The weight assigned to the supervised CE loss when using AKL."},
    )
    akl_distill_weight: float = field(
        default=1.0,
        metadata={"help": "The weight assigned to the AKL distillation loss."},
    )
    use_abkl_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the ABKL loss."},
    )
    use_todi_loss: bool = field(
        default=False,
        metadata={"help": "Whether to use the ToDi loss."},
    )
    abkl_temperature: float = field(
        default=1.0,
        metadata={"help": "The temperature parameter used by ABKL."},
    )
    abkl_alpha: float = field(
        default=1.0,
        metadata={"help": "The alpha parameter used by ABKL."},
    )
    abkl_beta: float = field(
        default=1.0,
        metadata={"help": "The beta parameter used by ABKL."},
    )
    todi_beta: float = field(
        default=1.0,
        metadata={"help": "The beta parameter used by ToDi for token-wise FKL/RKL weighting."},
    )
    todi_ce_weight: float = field(
        default=1.0,
        metadata={"help": "The weight assigned to the supervised CE loss when using ToDi."},
    )
    todi_distill_weight: float = field(
        default=1.0,
        metadata={"help": "The weight assigned to the ToDi distillation loss."},
    )
    ...
    def __post_init__(self):
        ...
        if self.use_hpd_loss and self.stage != "sft":
            raise ValueError("`use_hpd_loss` is only valid for SFT training.")

        if self.use_akl_loss and self.stage != "sft":
            raise ValueError("`use_akl_loss` is only valid for SFT training.")

        if self.use_abkl_loss and self.stage != "sft":
            raise ValueError("`use_abkl_loss` is only valid for SFT training.")

        if self.use_todi_loss and self.stage != "sft":
            raise ValueError("`use_todi_loss` is only valid for SFT training.")

        if not 0.0 < self.akl_mu <= 1.0:
            raise ValueError("`akl_mu` must be in the interval (0, 1].")

        if self.akl_ce_weight < 0.0:
            raise ValueError("`akl_ce_weight` must be non-negative.")

        if self.akl_distill_weight < 0.0:
            raise ValueError("`akl_distill_weight` must be non-negative.")

        if self.use_akl_loss and self.akl_ce_weight == 0.0 and self.akl_distill_weight == 0.0:
            raise ValueError("`akl_ce_weight` and `akl_distill_weight` cannot both be zero when using AKL.")

        if self.abkl_temperature <= 0.0:
            raise ValueError("`abkl_temperature` must be positive.")

        if self.todi_ce_weight < 0.0:
            raise ValueError("`todi_ce_weight` must be non-negative.")

        if self.todi_distill_weight < 0.0:
            raise ValueError("`todi_distill_weight` must be non-negative.")

        if self.use_todi_loss and self.todi_ce_weight == 0.0 and self.todi_distill_weight == 0.0:
            raise ValueError("`todi_ce_weight` and `todi_distill_weight` cannot both be zero when using ToDi.")

        if (
            int(self.use_dft_loss)
            + int(self.use_eaft_loss)
            + int(self.use_asft_loss)
            + int(self.use_hpd_loss)
            + int(self.use_akl_loss)
            + int(self.use_abkl_loss)
            + int(self.use_todi_loss)
            > 1
        ):
            raise ValueError(
                "`use_dft_loss`, `use_eaft_loss`, `use_asft_loss`, `use_hpd_loss`, `use_akl_loss`, `use_abkl_loss` and `use_todi_loss` are mutually exclusive."
            )

        if self.use_hpd_loss and self.finetuning_type == "lora" and self.ref_model is None:
            raise ValueError("`ref_model` is necessary for LoRA SFT training with HPD loss.")

        if self.use_akl_loss and self.finetuning_type == "lora" and self.ref_model is None:
            raise ValueError("`ref_model` is necessary for LoRA SFT training with AKL loss.")

        if self.use_abkl_loss and self.finetuning_type == "lora" and self.ref_model is None:
            raise ValueError("`ref_model` is necessary for LoRA SFT training with ABKL loss.")

        if self.use_todi_loss and self.finetuning_type == "lora" and self.ref_model is None:
            raise ValueError("`ref_model` is necessary for LoRA SFT training with ToDi loss.")
